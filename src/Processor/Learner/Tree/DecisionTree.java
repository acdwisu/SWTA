package Processor.Learner.Tree;
...
public class DecisionTree {
    ...
    */
    ...
    public DecisionTree(RandomForest forest, int treenum, ArrayList<String[]> knowledge) {
        // TODO Auto-generated constructor stub
        this.forest=forest;        
        importances = new int[RandomForest.M];       

        root=createTree(knowledge);
        flushData(root, treenum);
    }
    ...
    private void bootstrapSample(ArrayList<ArrayList<String>> data,ArrayList<ArrayList<String>> train,ArrayList<ArrayList<String>> test,int numb){
        ...
        //System.out.println("created testing-data for tree : "+numb);
...
//            System.out.println("Adding "+parent.ChildNode.size()+" children at level: "+parent.generation);
    ...
    public String toString() {
        ...
        res = this.traverseTreeNode(node, 1, res);
     ...
     * @return 
     */
    private String traverseTreeNode(TreeNode node, int level, String treeRep) {
        
                
        if(node.isLeaf) treeRep = treeRep.concat(level+ " " +this.treeNodeToString(node));
        else {                       
            treeRep = treeRep.concat(level+ " " +this.treeNodeToString(node));
            
            ArrayList<TreeNode> childs = node.ChildNode;
            
            for(TreeNode child : childs) {
                treeRep = this.traverseTreeNode(child, level+1, treeRep);
            }
        }
    ...
    private String treeNodeToString(TreeNode node) {
        ...
        if(node.isLeaf) {
            ...
            sb.append(node.label); 
            sb.append(" : "); 
            sb.append(node.splitAttributeM); 
            sb.append(" "); 
            sb.append(node.splitValue); 
            sb.append("\n");
        }
     ...
     * @return 
     */
    private TreeNode createTree(ArrayList<String[]> query) {
        TreeNode root = this.StringToTreeNode(query.get(0));

        if(query.size() > 1)        
            this.recursiveRetrieveKnowledge(root, query, 0);
        
        return root;
    }
    
    private void recursiveRetrieveKnowledge(TreeNode node, final ArrayList<String[]> query, int nodeIndexAtQuery) {        
        int[] indexChilds = this.getChildsIndex(query, nodeIndexAtQuery);

        ArrayList<TreeNode> childs = this.getChilds(query, indexChilds);

        node.ChildNode = childs;

        int i=0;
        for(TreeNode child : childs) {
            if(!child.isLeaf) 
                this.recursiveRetrieveKnowledge(child, query, indexChilds[i]);
     ...
     * 
     * @param query
     * @param nodeIndexAtQuery
     * @return 
     */
    private ArrayList<TreeNode> getChilds(ArrayList<String[]> query, int[] indexChilds) {
        ArrayList<TreeNode> childs = new ArrayList<>(2);
        
        childs.add(this.StringToTreeNode(query.get(indexChilds[0])));
        childs.add(this.StringToTreeNode(query.get(indexChilds[1])));
        
        return childs;
    }
    
    private int[] getChildsIndex(ArrayList<String[]> query, int nodeIndexAtQuery) {
        String[] nodeString = query.get(nodeIndexAtQuery);
        
        int parentLevel = Integer.parseInt(nodeString[0]);
        int[] indexChilds = new int[2];

        for(int i=nodeIndexAtQuery, found=0; found<2; i++) {
            int level = Integer.parseInt(query.get(i)[0]);
     ...
     * @return 
     */
    private TreeNode StringToTreeNode(String[] nodeString) {
        TreeNode node = new TreeNode();
        
        String label = nodeString[1];
        boolean isLeaf = false;
        String kelas = null, splitValue = null;
        ...
        switch(nodeString[2]) {
            case ":":
                isLeaf = false;
                kelas = "";
                splitAttribute = Integer.parseInt(nodeString[3]);
                splitValue = nodeString[4];                
                break;
            case "=":
                isLeaf = true;
                kelas = nodeString[3];
        ...
        node.isLeaf = isLeaf;
        node.Class = kelas;
        node.splitAttributeM = splitAttribute;
